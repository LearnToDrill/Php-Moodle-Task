...
class drill_usercourse_expiration_notification extends \core\task\scheduled_task
{
    ...
    public function execute()
    {
        ...
        $records = $DB->get_recordset_sql("SELECT coursename,deviceid,useremail,firstname,notifydate FROM vw_user_course_expiration_notification");
            ...
            foreach ($records as $id => $student) {
                ...
                if (time() == (string) ($data->notifydate)) {
                    ...
                    if (mail((string) ($data->useremail), $subject, $email_body, $header, '-f' . "info@learntodrill.com")) {
                        ...
                        \pushnotifications::iOS($msg_payload, (string) ($data->deviceid), $data->courseid);
                    } else {
                        exit;
                    }
                }
