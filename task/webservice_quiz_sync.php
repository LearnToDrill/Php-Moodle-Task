...
class local_quiz_sync_external extends external_api
{
    ...
    function finishedAttempt($params)
    {
        ...
        $result['state'] = $attemptobj->process_attempt($timenow, true);
        ...
        foreach ($slots as $slotno) {
            ...
            foreach ($arrayofAppdata as $element) {
                ...
                if ($moodleQid == $questionId) {
                    ...
                    if ($qType == "truefalse" || $qType == "shortanswer") { //Actual Qtype Keys pass here if tue false 0 false 1 true (Moodle standard)
    ...
    function prepareQuesAttempt($slot, $uniqId, $sequencecheck, $answerValue)
    {
        $questionData = array(
            "slots" => $slot,
            self::prepareSequencecheck($slot, $uniqId) => $sequencecheck,
            self::prepareAnswer($slot, $uniqId) => $answerValue
        );
        return $questionData;
    }
    ...
    function prepareSequencecheck($slotno, $uniqId)
    {
